...
export const useErrorHandler = () => {
  ...
  const logError = useCallback(
    (error: Error | string, contextData?: any) => {
  ...
  const logMessage = useCallback(
    (message: string, level?: ErrorLevel, contextData?: any) => {
  ...
  const logApiCall = useCallback(
    (method: string, url: string, status?: number, metadata?: any) => {
...
export const useAsyncError = () => {
  ...
  const handleAsyncError = useCallback(
    (asyncFn: (...args: any[]) => Promise<any>, context?: any) => {
...
export const useComponentError = (componentName: string) => {
  ...
  const logComponentError = useCallback(
    (error: Error, phase: string, metadata?: any) => {
...
export const useUserContext = () => {
  ...
  const identifyUser = useCallback(
    (id: string, email?: string, username?: string, metadata?: any) => {
      return setUser({
        id,
        email,
        username,
...
export const useErrorRetry = (maxRetries: number = 3) => {
  ...
  const retryWithBackoff = useCallback(
    async (fn: () => Promise<any>, context?: any) => {
...
export const useProviderManager = () => {
  ...
  const switchWithFallback = useCallback(
    async (provider: ErrorProviderType, config?: any, fallbackProvider?: ErrorProviderType) => {
      try {
